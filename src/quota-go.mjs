...
// (src/gate.mjs). Legge lo stesso meccanismo usato da un'estensione separata
// per il footer di `pi` (progetto a parte, non incluso qui), senza duplicarne
// la logica da zero.
//
// Verificato dal vivo: la lettura del dato sta tutta in funzioni pure
// (estraiDallaPagina/scarica — fetch di opencode.ai con il cookie auth,
// estrazione dal JS della pagina); il contesto dell'estensione originale
// serviva solo a mostrare il footer. Quindi il meccanismo è richiamabile da
// un processo Node standalone, e qui è copiato fedelmente con attribuzione:
// stessa pagina, stesso cookie, stesso parsing, stesso file di credenziali.
// Non è un secondo tracciamento della quota — è lo stesso dato, letto dalla
// stessa fonte.
//
// Non si importa l'estensione originale direttamente: legherebbe superPi a un
// repository esterno e al supporto TS sperimentale di Node. La copia è
// pinnata dal test fixture in scripts/test-fase4.mjs: se il formato della
// pagina cambia, il test lo scopre.
//
// Credenziali (MAI stampate, MAI committate — stesse regole dell'estensione):
//   OPENCODE_GO_AUTH_COOKIE    cookie auth di opencode.ai
//   OPENCODE_GO_WORKSPACE_ID   id del TUO workspace OpenCode Go, obbligatorio
// da ambiente oppure dal file ~/.config/pi/uso-go.env (chmod 600, fuori da
// qualsiasi repo). Se il cookie è esposto: ruotarlo. Nessun default: un
// workspace id sbagliato leggerebbe la quota di qualcun altro, quindi va
// dichiarato esplicitamente (stesso principio fail-closed del gate).
...
export async function ottieniQuotaGo() {
  ...
  if (!workspaceId) {
    throw new Error("OPENCODE_GO_WORKSPACE_ID non impostato (né in ambiente né in ~/.config/pi/uso-go.env)");
  }
